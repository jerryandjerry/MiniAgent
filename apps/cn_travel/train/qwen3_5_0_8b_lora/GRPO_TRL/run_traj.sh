#!/usr/bin/env bash
# Read train_traj_config.json, resolve paths from the application root, then launch train_traj.py from this directory.
set -euo pipefail
...
cfg_path = os.environ.get("CONFIG", os.path.join(d, "train_traj_config.json"))
if not pathlib.Path(cfg_path).is_absolute():
    cfg_path = str(app_root / cfg_path)
cfg = json.load(open(cfg_path, encoding="utf-8"))
cfg.pop("mining", None)
flags = cfg.pop("flags", [])
for key in ("train_file", "model_name_or_path", "sft_adapter", "output_dir",
            "tools_file", "task_filter"):
    if cfg.get(key) and not pathlib.Path(cfg[key]).is_absolute():
        cfg[key] = str(app_root / cfg[key])
cmd = [sys.executable, os.path.join(d, "train_traj.py")]
